export class ViewportModel {
  ...
  public fromJson(json: any): void {
    ...
    this.items = (json.items || []).map((node: any) => {
      return new ItemModel(node.name || '', node.type || 0).setPosition(node.x, node.y).setSize(node.width, node.height).setColor(node.color);
    });
...
export class ItemModel {
  ...
  public setColor(color: any): ItemModel {
    this.color = color;
    return this;
  }
...
export class ConnectionModel {
  ...
  public setColor(color: any): ConnectionModel {
    this.color = color;
    return this;
  }
